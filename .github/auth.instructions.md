...
# 🔒 Core Security Conventions (All Methods)
...
30. **Single-Use Tokens:** Magic link tokens **MUST** be single-use. Immediately delete the token from Redis after validation (current implementation: ✅ correct).
...
37. **State Parameter:** Always generate and verify the **`state`** parameter using `arctic.generateState()` to prevent CSRF attacks (current implementation: ✅ correct).
...
38. **State Storage:** Store OAuth state in Redis with:
    ...
    - TTL: 10 minutes (current implementation: ✅ correct)
...
40. **Code Verifier (PKCE):** Always use PKCE (Proof Key for Code Exchange) with `arctic.generateCodeVerifier()`. This protects against authorization code interception attacks (current implementation: ✅ correct).
...
51. **Cascade Deletion:** Use `onDelete: 'cascade'` for user-owned resources (sessions, identities, magic links) to ensure cleanup when users are deleted (current implementation: ✅ correct).
...
53. **Index Foreign Keys:** All foreign keys (userId) **MUST** be indexed for query performance (current implementation: ✅ correct).
...
export const passwordSchema = pipe(
  ...
  custom(
    ...
    "Heslo musí obsahovat velkć písmeno, malé písmeno, číslo a speciální znak"
  )
);
```
