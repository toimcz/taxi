 ...
 */
export const idempotencyKeySchema = (message?: string) => pipe(string(), trim(), uuid(message));
...
type ResultWithIdempotencyKey<T extends Schema> =
	| {
			issues: undefined;
			output: InferOutput<T>;
...
export const validateRequest = async <T extends Schema>(
	schema: T,
	request: Request,
): Promise<ResultWithIdempotencyKey<T>> => {
...
export const validateQuery = async <T extends Schema>(schema: T, url: URL): Promise<Result<T>> => {
	const searchParams = Object.fromEntries(url.searchParams);
	const validated = safeParse(schema, searchParams);
	if (!validated.success) {
		const issues = (flatten(validated.issues).nested ?? {}) as Issues<T>;
		return { issues, output: undefined };
	}
