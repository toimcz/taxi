  ...
  <Card>
    <div class="flex justify-between">
      ...
      <div class="flex items-center gap-x-3">
        <div class="text-slate-600">Počet: {data.bases.meta?.total ?? 0}</div>
        {#if data.auth.roles.includes('DEV')}
    ...
    <div class="datatable">
      ...
      {#each data.bases.data as base (base.id)}
        <a href="/zakladny/{base.id}" class="tr">
          ...
          <div class="td" class:text-success={base.status} class:text-danger={!base.status}>
    ...
    <div class="grid grid-cols-1 gap-y-1 lg:hidden">
      {#each data.bases.data as base (base.id)}
        <a href="/zakladny/{base.id}" class="flex justify-between gap-x-2">
          <div>{base.city}</div>
          <div class:text-success={base.status} class:text-danger={!base.status}>
            {base.status ? 'Aktivní' : 'Neaktivní'}
          </div>
        </a>
      {/each}
