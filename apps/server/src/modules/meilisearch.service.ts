	...
	async checkMeiliSearchHealth() {
		try {
			...
			if (health.status === "available") {
				logger.info("MeiliSearch is healthy and available");
			} else {
				logger.error(`MeiliSearch is not available: ${JSON.stringify(health)}`);
				process.exit(1);
			}
		} catch (error) {
...
> extends MeiliSearchClient {
	...
	async init(data: T): Promise<void> {
		...
		if (!index) {
			logger.warn(`Failed to create MeiliSearch index ${indexName}`);
			return;
		}
		await this.waitForIndexExists(indexName);
		await this.configureIndexSettings();
		await this.addAll(data);
		logger.info(`MeiliSearch index ${indexName} initialized successfully`);
	}

	private async waitForIndexExists(
		uid: string,
		timeoutMs = 10_000,
	): Promise<void> {
		...
		throw new Error(
			`Timeout waiting for MeiliSearch index ${uid} to be created`,
		);
	}

	private async waitForIndexNotExists(
		uid: string,
		timeoutMs = 10_000,
	): Promise<void> {
		const start = Date.now();
		...
		throw new Error(
			`Timeout waiting for MeiliSearch index ${uid} to be deleted`,
		);
	}
	...
	private async configureIndexSettings(): Promise<void> {
		try {
			...
			logger.warn("Failed to configure index settings:");
			logger.warn(error);
		}
	}
	...
	private async addAll(data: T): Promise<void> {
		try {
			logger.info("Adding all routes to MeiliSearch");
			if (!data.length) {
				logger.warn("No routes found to add to MeiliSearch");
				return;
			}
			...
			logger.info(`All added to MeiliSearch index ${this.options.indexName}`);
		} catch (error) {
			logger.error(
				`Failed to add all to MeiliSearch: ${this.options.indexName}`,
			);
			logger.error(error);
		}
	}
	...
	async addOne(item: T[number]): Promise<void> {
		try {
			...
			logger.error("Failed to add item to MeiliSearch:");
			logger.error(error);
		}
	}
	...
	async updateOne(item: T[number]): Promise<void> {
		try {
			...
			logger.error("Failed to update item in MeiliSearch:");
			logger.error(error);
		}
	}
	...
	async deleteOne(id: string | number): Promise<void> {
		try {
			...
			logger.error("Failed to delete item from MeiliSearch:");
			logger.error(error);
		}
	}
	...
	async deleteAll(): Promise<void> {
		try {
			...
			logger.error("Failed to delete all items from MeiliSearch:");
			logger.error(error);
		}
	}
	...
	async search(query: string): Promise<T> {
		try {
			...
			logger.error("Failed to search MeiliSearch:");
			logger.error(error);
